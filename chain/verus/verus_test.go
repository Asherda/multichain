...
var _ = Describe("Verus", func() {
	Context("when submitting transactions", func() {
		Context("when sending Verus to multiple addresses", func() {
			It("should work", func() {
				// Load private key, and assume that the associated address has
				// funds to spend. You can do this by setting VERUS_PK to
				// the value specified in the `./multichaindeploy/.env` file.
				pkEnv := os.Getenv("VERUS_PK")
				if pkEnv == "" {
					panic("VERUS_PK is undefined")
				}
				wif, err := btcutil.DecodeWIF(pkEnv)
				Expect(err).ToNot(HaveOccurred())

				// PKH
				pkhAddr, err := verus.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeCompressed()), &verus.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				pkhAddrUncompressed, err := verus.NewAddressPubKeyHash(btcutil.Hash160(wif.PrivKey.PubKey().SerializeUncompressed()), &verus.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				...
				tx, err := verus.NewTxBuilder(&verus.RegressionNetParams, 1000000).BuildTx(inputs, recipients)
				Expect(err).ToNot(HaveOccurred())
